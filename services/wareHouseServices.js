...
const getWareHouses = async (filter = {}, search) => {
  if (search) {
    filter.$or = [{ wareHouseCode: new RegExp(search, "i") }];
  }
...
const scanMultipleProducts = async (Product_Codes, wareHouse_code, userId) => {
  ...
  try {
    ...
    if (!ware_house) {
      errorMessages.push(
        `No Warehouse found for the given code: ${wareHouse_code}`
      );
    }
    ...
    if (missingProductCodes.length > 0) {
      messages.push(
        `No products found for given codes: ${missingProductCodes.join(", ")}`
      );
    }
    ...
    const ExhaustedProductCodes = ExhaustedProducts.map(
      (p) => p.productCode
    ).join(", ");
    ...
    if (
      ...
      const newwareHouseProducts = ExhaustedProductIds.filter(
        (id) => !wareHouseProducts.includes(id)
      );

      ware_house.products = [...wareHouseProducts, ...newwareHouseProducts];
      await ware_house.save({ session });
      ...
      const genrateLogForNew = {
        ...
        productNotes: "Managed By WareHouse",
      };

      await Log.createLog(genrateLogForNew, session);

      messages.push(
        `Product status updated to NEW for: ${ExhaustedProductCodes}`
      );
      success = true;
    }
